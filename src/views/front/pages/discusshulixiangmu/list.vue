...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
			...
			<view class="list" :style='{"width":"calc(100% - 48rpx)","padding":"32rpx 24rpx 24rpx","margin":"40rpx auto 0","borderRadius":"24rpx","background":"#ffffff","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"border":"0px solid #eee","padding":"20rpx","margin":"0 0 40rpx","borderRadius":"12rpx","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusshulixiangmu','修改')) || (!userid && isAuthFront('discusshulixiangmu','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusshulixiangmu','删除')) || (!userid && isAuthFront('discusshulixiangmu','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
					...
					<!-- #ifndef MP-WEIXIN -->
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusshulixiangmu','修改')) || (!userid && isAuthFront('discusshulixiangmu','修改'))" @tap.stop.proevent="onUpdateTap(product)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
						</view>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discusshulixiangmu','删除')) || (!userid && isAuthFront('discusshulixiangmu','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
						</view>
					</view>
		...
		<button :style='{"border":"0","boxShadow":"none","color":"#38bf3e","right":"40rpx","borderRadius":"100%","top":"112rpx","background":"#fff","width":"72rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"72rpx","zIndex":"999"}' v-if="userid && isAuth('discusshulixiangmu','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"none","color":"#38bf3e","right":"40rpx","borderRadius":"100%","top":"112rpx","background":"#fff","width":"72rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"72rpx","zIndex":"999"}' v-if="!userid && isAuthFront('discusshulixiangmu','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`discusshulixiangmu`, params);
                } else {
                    res = await this.$api.list(`discusshulixiangmu`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('discusshulixiangmu', JSON.stringify([id]));
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`discusshulixiangmu`, searchForm);
                } else {
                    res = await this.$api.list(`discusshulixiangmu`, searchForm);
                }
				// 如果是第一页数据置空
