...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusschuangweixinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusschuangweixinxiCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discusschuangweixinxi', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discusschuangweixinxi');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','discusschuangweixinxi');
			...
			async init(type=1){
				...
				let res = await this.$api.info('discusschuangweixinxi', this.id);
				this.detail = res.data;

				if(type==2){
					this.detail.discussnum++
					await this.$api.update('discusschuangweixinxi',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscusschuangweixinxi', {
						page: mescroll.num,
						limit: 10,
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscusschuangweixinxi/add-or-update?refid=${this.id}`)
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscusschuangweixinxi',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discusschuangweixinxi',that.detail)
							setTimeout(()=>{
