<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('chuangweixinxi','预订')" @click="chuangweiyudingCrossAddOrUpdateHandler(scope.row,'cross','','','zhuangtai','已被占用','占用,空闲'.split(',')[0])" type="success">
							...
							<el-button class="view" v-if="isAuth('chuangweixinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看评论
							</el-button>
		...
		<chuangweiyuding-cross-add-or-update v-if="chuangweiyudingCrossAddOrUpdateFlag" :parent="this" ref="chuangweiyudingCrossaddOrUpdate"></chuangweiyuding-cross-add-or-update>
...
import chuangweiyudingCrossAddOrUpdate from "../chuangweiyuding/add-or-update";
	export default {
		data() {
			return {
				zhuangtaiOptions: [],
				...
				chuangweiyudingCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			chuangweiyudingCrossAddOrUpdate,
		},
		methods: {
			contentStyleChange() {
			...
			chuangweiyudingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.chuangweiyudingCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.chuangweiyudingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.chuangweiyudingCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    init () {
          this.zhuangtaiOptions = "占用,空闲".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
	...
	this.$router.push({path:'/discusschuangweixinxi',query:{refid:id}});
    },
    // 下载
    download(file){
