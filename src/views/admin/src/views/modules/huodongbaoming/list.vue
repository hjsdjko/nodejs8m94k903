<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="kebaorenshuChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var kebaorenshuChart1 = echarts.init(document.getElementById("kebaorenshuChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                kebaorenshuChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    kebaorenshuChart1.resize();
                };
            }
        });
      })
	...
	huodongbaomingstatusChange(e,row){
		if(row.status==0){
