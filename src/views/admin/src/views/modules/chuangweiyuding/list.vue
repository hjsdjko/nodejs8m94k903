<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#333","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('chuangweiyuding','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('chuangweiyuding','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('chuangweiyuding','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('chuangweiyuding','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
			...
			<div :style='{"border":"0px solid #eee","padding":"0","margin":"10px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)","width":"100%","fontSize":"12px"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('chuangweiyuding','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('chuangweiyuding','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('chuangweiyuding','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('chuangweiyuding','紧急呼叫')" @click="jinjihujiaoCrossAddOrUpdateHandler(scope.row,'cross','是','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								紧急呼叫
							</el-button>
							<el-button class="btn8" v-if="isAuth('chuangweiyuding','健康汇报')" @click="jiankangshujuCrossAddOrUpdateHandler(scope.row,'cross','是','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								健康汇报
							</el-button>
							<el-button class="btn8" v-if="isAuth('chuangweiyuding','入住评价')" @click="ruzhupingjiaCrossAddOrUpdateHandler(scope.row,'cross','是','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								入住评价
							</el-button>
							<el-button class="edit" v-if=" isAuth('chuangweiyuding','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('chuangweiyuding','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<jinjihujiao-cross-add-or-update v-if="jinjihujiaoCrossAddOrUpdateFlag" :parent="this" ref="jinjihujiaoCrossaddOrUpdate"></jinjihujiao-cross-add-or-update>
		<jiankangshuju-cross-add-or-update v-if="jiankangshujuCrossAddOrUpdateFlag" :parent="this" ref="jiankangshujuCrossaddOrUpdate"></jiankangshuju-cross-add-or-update>
...
import jinjihujiaoCrossAddOrUpdate from "../jinjihujiao/add-or-update";
import jiankangshujuCrossAddOrUpdate from "../jiankangshuju/add-or-update";
import ruzhupingjiaCrossAddOrUpdate from "../ruzhupingjia/add-or-update";
	export default {
		data() {
			return {
				...
				jinjihujiaoCrossAddOrUpdateFlag: false,
				jiankangshujuCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			jinjihujiaoCrossAddOrUpdate,
			jiankangshujuCrossAddOrUpdate,
		...
		methods: {
			...
			jinjihujiaoCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.jinjihujiaoCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','chuangweiyuding');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jinjihujiaoCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jinjihujiaoCrossaddOrUpdate.init(row.id,type);
				});
			},
			jiankangshujuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.jiankangshujuCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','chuangweiyuding');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jiankangshujuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jiankangshujuCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			payHandler(row){
				...
				this.$storage.set('paytable','chuangweiyuding');
		...
		this.$refs["shBatchForm"].validate(valid => {
			if(valid){
				this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
					...
					this.$http({
						url: "chuangweiyuding/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
						method: "post",
						data: this.batchIds
					}).then(({ data }) => {
						if (data && data.code === 0) {
	...
	chuangweiyudingstatusChange(e,row){
		if(row.status==0){
