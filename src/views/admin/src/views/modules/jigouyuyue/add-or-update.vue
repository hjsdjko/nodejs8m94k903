	...
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"14px","color":"#666","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="机构地址" prop="jigoudizhi">
					<el-input v-model="ruleForm.jigoudizhi" placeholder="机构地址" clearable  :readonly="ro.jigoudizhi"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="机构地址" prop="jigoudizhi">
					<el-input v-model="ruleForm.jigoudizhi" placeholder="机构地址" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="upload" v-if="type!='info' && !ro.jigoutupian" label="机构图片" prop="jigoutupian">
					<file-upload
						tip="点击上传机构图片"
						action="file/upload"
						:limit="3"
						:multiple="true"
						:fileUrls="ruleForm.jigoutupian?ruleForm.jigoutupian:''"
						@change="jigoutupianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="upload" v-else-if="ruleForm.jigoutupian" label="机构图片" prop="jigoutupian">
					<img v-if="ruleForm.jigoutupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.jigoutupian.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.jigoutupian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='jigoutupian'){
							this.ruleForm.jigoutupian = obj[o];
							this.ro.jigoutupian = true;
							continue;
						}
						if(o=='yuyueshijian'){
    ...
    // 提交
    onSubmit() {
	if(this.ruleForm.jigoutupian!=null) {
		this.ruleForm.jigoutupian = this.ruleForm.jigoutupian.replace(new RegExp(this.$base.url,"g"),"");
	}
var objcross = this.$storage.getObj('crossObj');
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `jigouyuyue/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jigouyuyueCrossAddOrUpdateFlag = false;
											this.parent.search();
    ...
    jigoutupianUploadChange(fileUrls) {
	    this.ruleForm.jigoutupian = fileUrls;
    },
  }
};
</script>
