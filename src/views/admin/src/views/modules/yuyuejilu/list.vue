<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","textAlign":"left","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">机构名称</label>
						<el-input v-model="searchForm.jigoumingcheng" placeholder="机构名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">护理项目</label>
						<el-input v-model="searchForm.hulixiangmu" placeholder="护理项目" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#333","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('yuyuejilu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('yuyuejilu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yuyuejilu','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('yuyuejilu','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-xihuan" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						批量支付
					</el-button>

					<el-button class="btn18" v-if="isAuth('yuyuejilu','服务预约')" type="success" @click="chartDialog1()">
			...
			<div :style='{"border":"0px solid #eee","padding":"0","margin":"10px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)","width":"100%","fontSize":"12px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='true' prop="hulitupian" width="200" label="护理图片">
						<template slot-scope="scope">
							<div v-if="scope.row.hulitupian">
								<img v-if="scope.row.hulitupian.substring(0,4)=='http'" :src="scope.row.hulitupian.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.hulitupian.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('yuyuejilu','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yuyuejilu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuyuejilu','护理评价')" @click="hulipingjiaCrossAddOrUpdateHandler(scope.row,'cross','是','是','[1]','已评价')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								护理评价
							</el-button>
							<el-button class="edit" v-if=" isAuth('yuyuejilu','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('yuyuejilu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<hulipingjia-cross-add-or-update v-if="hulipingjiaCrossAddOrUpdateFlag" :parent="this" ref="hulipingjiaCrossaddOrUpdate"></hulipingjia-cross-add-or-update>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="hulixiangmuChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
import hulipingjiaCrossAddOrUpdate from "../hulipingjia/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				hulipingjiaCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			hulipingjiaCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.hulipingjiaCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yuyuejilu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.hulipingjiaCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.hulipingjiaCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				if(row.sfsh!='是'){
				...
				this.$storage.set('paytable','yuyuejilu');
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var hulixiangmuChart1 = echarts.init(document.getElementById("hulixiangmuChart1"),'macarons');
        this.$http({
            url: "yuyuejilu/group/hulixiangmu",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].hulixiangmu);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].hulixiangmu
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                hulixiangmuChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    hulixiangmuChart1.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.jigoumingcheng!='' && this.searchForm.jigoumingcheng!=undefined){
            params['jigoumingcheng'] = '%' + this.searchForm.jigoumingcheng + '%'
          }
           if(this.searchForm.hulixiangmu!='' && this.searchForm.hulixiangmu!=undefined){
		...
		this.$refs["shBatchForm"].validate(valid => {
			if(valid){
				this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
					...
					this.$http({
						url: "yuyuejilu/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
						method: "post",
						data: this.batchIds
					}).then(({ data }) => {
						if (data && data.code === 0) {
