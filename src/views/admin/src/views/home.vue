<template>
<div class="content" :style='{"minHeight":"100vh","padding":"0px 20px 20px","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230831/a5f221858a6744baab19924fb77cee81.png) no-repeat center top / cover","display":"flex","width":"100%","fontSize":"14px","height":"auto"}'>
	<!-- notice -->
	<!-- title -->
	...
	<div :style='{"border":"0px solid rgba(126, 96, 16, .1)","padding":"0px 0","margin":"0 30px 30px 0px","alignItems":"center","color":"#666","flexWrap":"wrap","background":"none","display":"flex","width":"250px","fontSize":"14px","justifyContent":"center","order":"0"}'>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"8px 0","margin":"0 0 10px 0","borderColor":"rgba(126, 96, 16, .1)","alignItems":"center","textAlign":"center","display":"flex","justifyContent":"space-around","borderRadius":"0px","background":"#e7e8f8","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid"}' v-if="isAuth('huodongbaoming','首页总数')">
			...
			<div :style='{"width":"auto","flexDirection":"column-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"bold","height":"24px"}'>{{huodongbaomingCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>活动报名总数</div>
			</div>
		</div>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"8px 0","margin":"0 0 10px 0","borderColor":"rgba(126, 96, 16, .1)","alignItems":"center","textAlign":"center","display":"flex","justifyContent":"space-around","borderRadius":"0px","background":"#e7f8f8","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid"}' v-if="isAuth('jigouyuyue','首页总数')">
			<div :style='{"alignItems":"center","borderRadius":"100%","background":"#fff","display":"flex","width":"48px","justifyContent":"center","height":"48px"}'>
				<span class="icon iconfont icon-tongji18" :style='{"color":"#ceeded","fontSize":"32px"}'></span>
			</div>
			<div :style='{"width":"auto","flexDirection":"column-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"bold","height":"24px"}'>{{jigouyuyueCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>机构预约总数</div>
			</div>
		</div>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"8px 0","margin":"0 0 10px 0","borderColor":"rgba(126, 96, 16, .1)","alignItems":"center","textAlign":"center","display":"flex","justifyContent":"space-around","borderRadius":"0px","background":"#e7f8e8","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid"}' v-if="isAuth('yuyuejilu','首页总数')">
			<div :style='{"alignItems":"center","borderRadius":"100%","background":"#fff","display":"flex","width":"48px","justifyContent":"center","height":"48px"}'>
				<span class="icon iconfont icon-tongji18" :style='{"color":"#ceedd0","fontSize":"32px"}'></span>
			</div>
			<div :style='{"width":"auto","flexDirection":"column-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"bold","height":"24px"}'>{{yuyuejiluCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>预约记录总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type3" :style='{"alignContent":"flex-start","padding":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"420px","order":"10"}'>
		<div id="huodongbaomingChart1" class="echarts1" v-if="isAuth('huodongbaoming','首页统计')"></div>
		<div id="jigouyuyueChart1" class="echarts2" v-if="isAuth('jigouyuyue','首页统计')"></div>
		<div id="yuyuejiluChart1" class="echarts3" v-if="isAuth('yuyuejilu','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		this.getjigouyuyueCount();
		this.jigouyuyueChat1();
		this.getyuyuejiluCount();
		this.yuyuejiluChat1();
	},
	methods:{
		// 词云
        ...
        this.$http({
            url: `huodongbaoming/value/huodongmingcheng/kebaorenshu`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
		...
		getjigouyuyueCount() {
			this.$http({
				url: `jigouyuyue/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.jigouyuyueCount = data.data
				}
			})
		},
		jigouyuyueChat1() {
        ...
        var jigouyuyueChart1 = echarts.init(document.getElementById("jigouyuyueChart1"),'macarons');
        this.$http({
            url: "jigouyuyue/group/jigoumingcheng",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                jigouyuyueChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    jigouyuyueChart1.resize();
                };
            }
        });
      })
        ...
        var yuyuejiluChart1 = echarts.init(document.getElementById("yuyuejiluChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                yuyuejiluChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yuyuejiluChart1.resize();
                };
            }
        });
      })
