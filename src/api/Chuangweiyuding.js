...
import ChuangweiyudingModel from '../models/ChuangweiyudingModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let tianshu = req.query.tianshu
			if (tianshu) {

				if (tianshu.indexOf('%') != -1) {
					where.tianshu = {
						[Op.like]: tianshu
					}
				} else {
			...
			let zongjia = req.query.zongjia
			if (zongjia) {

				if (zongjia.indexOf('%') != -1) {
					where.zongjia = {
						[Op.like]: zongjia
					}
				} else {
			...
			let hujiaodianhua = req.query.hujiaodianhua
			if (hujiaodianhua) {

				if (hujiaodianhua.indexOf('%') != -1) {
					where.hujiaodianhua = {
						[Op.like]: hujiaodianhua
					}
				} else {
					where.hujiaodianhua = {
						[Op.eq]: hujiaodianhua
			...
			let yudingbeizhu = req.query.yudingbeizhu
			if (yudingbeizhu) {

				if (yudingbeizhu.indexOf('%') != -1) {
					where.yudingbeizhu = {
						[Op.like]: yudingbeizhu
					}
				} else {
					where.yudingbeizhu = {
						[Op.eq]: yudingbeizhu
			...
			if(tableName == 'yanglaojigou') {
				where.jigoumingcheng = {
					[Op.eq]: req.session.userinfo === undefined ? jwt.decode(req.headers.token).jigoumingcheng : req.session.userinfo.jigoumingcheng
				}
			...
			if(tableName == 'jiashu') {
				where.jiashuzhanghao = {
					[Op.eq]: req.session.userinfo === undefined ? jwt.decode(req.headers.token).jiashuzhanghao : req.session.userinfo.jiashuzhanghao
				}
				if (where['userid'] != undefined) {
					delete where.userid
				}
			...
			let result = await ChuangweiyudingModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await ChuangweiyudingModel.findAll()
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await ChuangweiyudingModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await ChuangweiyudingModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete', async (req, res) => {
		...
		try {
			...
			await ChuangweiyudingModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await ChuangweiyudingModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		let tableName = req.session.userinfo === undefined ? jwt.decode(req.headers.token).tableName : req.session.userinfo.tableName
        if(tableName == 'yanglaojigou') {
            where += " AND jigoumingcheng = '" + jwt.decode(req.headers.token).username + "' ";
        }
        if(tableName == 'yonghu') {
            where += " AND yonghuzhanghao = '" + jwt.decode(req.headers.token).username + "' ";
        }
        if(tableName == 'jiashu') {
            where += " AND jiashuzhanghao = '" + jwt.decode(req.headers.token).username + "' ";
        }
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM chuangweiyuding WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM chuangweiyuding WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM chuangweiyuding WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM chuangweiyuding WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM chuangweiyuding WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM chuangweiyuding WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM chuangweiyuding " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			if ("chuangweiyuding" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM chuangweiyuding " + where + " GROUP BY " + xColumnName + " DESC"
	...
	api.get('/valueMul/:xColumnName', async (req, res) => {
		...
		try {	
			...
			if(userTableName == 'yanglaojigou') {
				where += " AND jigoumingcheng = '" + jwt.decode(req.headers.token).username + "' ";
			}
			if(userTableName == 'yonghu') {
				where += " AND yonghuzhanghao = '" + jwt.decode(req.headers.token).username + "' ";
			}
			if(userTableName == 'jiashu') {
				where += " AND jiashuzhanghao = '" + jwt.decode(req.headers.token).username + "' ";
			}
	...
	api.post('/shBatch', async (req, res) => {
		...
		try {
			...
			req.body.forEach(async id => {
				let data= await ChuangweiyudingModel.findOne({ where: { id } })
				data = data.dataValues
				data.sfsh = req.query.sfsh
				data.shhf = req.query.shhf
				await ChuangweiyudingModel.update(data, {
					where: {
						id: data.id || 0
					}
				})
