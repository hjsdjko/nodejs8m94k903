...
import YuyuejiluModel from '../models/YuyuejiluModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let hulixiangmu = req.query.hulixiangmu
			if (hulixiangmu) {

				if (hulixiangmu.indexOf('%') != -1) {
					where.hulixiangmu = {
						[Op.like]: hulixiangmu
					}
				} else {
			...
			let hulijiage = req.query.hulijiage
			if (hulijiage) {

				if (hulijiage.indexOf('%') != -1) {
					where.hulijiage = {
						[Op.like]: hulijiage
					}
				} else {
			...
			let hulitupian = req.query.hulitupian
			if (hulitupian) {

				if (hulitupian.indexOf('%') != -1) {
					where.hulitupian = {
						[Op.like]: hulitupian
					}
				} else {
			...
			let yuyuebeizhu = req.query.yuyuebeizhu
			if (yuyuebeizhu) {

				if (yuyuebeizhu.indexOf('%') != -1) {
					where.yuyuebeizhu = {
						[Op.like]: yuyuebeizhu
					}
				} else {
			...
			let result = await YuyuejiluModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await YuyuejiluModel.findAll()
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await YuyuejiluModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await YuyuejiluModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await YuyuejiluModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM yuyuejilu WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM yuyuejilu WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM yuyuejilu WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM yuyuejilu WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM yuyuejilu WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM yuyuejilu WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM yuyuejilu " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM yuyuejilu " + where + " GROUP BY " + xColumnName + " DESC"
	...
	api.get('/count', async (req, res) => {
		...
		try {
			let where = {}
			var jigoumingcheng = req.query.jigoumingcheng
			if (jigoumingcheng) {
			...
			var hulixiangmu = req.query.hulixiangmu
			if (hulixiangmu) {
			...
			const count = await YuyuejiluModel.count({where});
	...
	api.post('/shBatch', async (req, res) => {
		...
		try {
			...
			req.body.forEach(async id => {
				let data= await YuyuejiluModel.findOne({ where: { id } })
				data = data.dataValues
				data.sfsh = req.query.sfsh
				data.shhf = req.query.shhf
				await YuyuejiluModel.update(data, {
					where: {
						id: data.id || 0
					}
				})
