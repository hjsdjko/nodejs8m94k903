...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let baomingbeizhu = req.query.baomingbeizhu
			if (baomingbeizhu) {

				if (baomingbeizhu.indexOf('%') != -1) {
					where.baomingbeizhu = {
						[Op.like]: baomingbeizhu
					}
				} else {
					where.baomingbeizhu = {
						[Op.eq]: baomingbeizhu
	...
	api.get('/count', async (req, res) => {
		...
		try {
			...
			var huodongleixing = req.query.huodongleixing
			if (huodongleixing) {
			...
			var huodongdidian = req.query.huodongdidian
			if (huodongdidian) {
